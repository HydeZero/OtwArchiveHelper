...
public class OtwArchiveHelper
{
    ...
    private async Task<Dictionary<string, string>> GetWorkPageBackend(string workId, string chapterId="")
    {
        ...
        string url = $"{ArchivePath}works/{workId}/chapters/{chapterId}?view_adult=true"; // construct the URL for the work page.
        ...
        var workContent = workPage.DocumentNode.SelectSingleNode("//div[@id='workskin']");
        if (workContent == null)
        {
        ...
        catch (NullReferenceException)
        {
            try
            {
                text = workContent.SelectSingleNode(".//div[@id='chapters']/div[@class='chapter']").InnerHtml.Trim();
            }
            catch (NullReferenceException)
            {
        ...
        try
        {
            var nextChapterTag = workPage.DocumentNode.SelectSingleNode("/html/body/div[@id='outer']/div[@id='inner']/div[@id='main']/div[@id='feedback']/ul[@class='actions']/li/*[contains(text(), 'Next Chapter')]").Attributes["href"].Value;
            nextChapter = nextChapterTag.Split("/").Last().Split("#")[0];
        }
        catch (Exception)
        {
